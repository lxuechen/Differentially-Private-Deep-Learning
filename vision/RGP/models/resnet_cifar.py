...
def lrk_conv3x3(in_planes, out_planes, stride=1, rank=1, group=1, kernel_size=3, padding=1):
    return LrkConv2d(in_planes, out_planes, stride=stride, rank=rank, group=group, kernel_size=kernel_size,
                     padding=padding)
...
class LrkConv2d(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride, rank=1, group=1, kernel_size=3, padding=1):
        ...
        # We find that only using the gradients of the left gradient carrier further reduces the number of trainable
...
def LinearBackwardHook(module, grad_input, grad_output, loss_reduction='mean'):
    ...
    # scale down the grad of the final classification layer so that it has approximatly the same norm as low-rank
    # gradients
